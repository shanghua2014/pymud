...
class CmdWalk(Command, IConfig):
    ...
    def __init__(self, session, *args, **kwargs):
        kwargs.setdefault("id", "cmd_walk")
        super().__init__(session, r"^walk(?:\s(\S+))?(?:\s(\S+))?(?:\s(\S+))?", *args, **kwargs)
        ...
        try:
            ...
            if m:
                ...
                if (len(para) > 0) and para[-1].isnumeric():
                    ...
                    while cnt > 0:
                        cnt = cnt - 1
                        await self.session.tris.tri_room.triggered()
