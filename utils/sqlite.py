...
class DatabaseManager:
    ...
    def select_room(self, room_name: str = None, room_id: int = None, table_name: str = None) -> Optional[Dict[str, Any]]:
        """
        查询房间记录（查）
    ...
    def delete_room(self, room_id: int = None, room_name: str = None, table_name: str = None) -> bool:
        """
        删除房间记录（删）
    ...
    # 保持向后兼容的方法
    def get_room_info(self, room_name: str, table_name: str = None) -> Dict[str, Any]:
        """
        根据房间名称获取房间信息（向后兼容）
    ...
    def save_room_info(self, room_name: str, desc: str, table_name: str = None) -> bool:
        ...
        # 检查房间是否已存在
        existing_room = self.select_room(room_name=room_name, table_name=table_name)
...
if __name__ == "__main__":
    ...
    if db.connect():
        ...
        # 查询测试数据
        room = db.select_room(room_name="测试房间")
        if room:
            print(f"✓ 查询成功: {room}")
